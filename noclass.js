...
function Color(color) {
  ...
  function from(color, type=null) {
    if (!color) {
      throw new Error('No color provided')
    } else if (typeof color === 'string') {
      fromString(color, type)
    } else if (typeof color === 'number') {
      fromNumber(color, type)
    } else if (Array.isArray(color)) {
      fromArray(color, type)
    } else {
      fromObject(color, type)
    }
  }

  function fromString(color, type=null) {
    ...
    if ((match = matchers.rgb.exec(color))) {
      _color = ensureRGB(match[1], match[2], match[3])
      _mode = RGB
    } else if ((match = matchers.rgba.exec(color))) {
      _color = ensureRGB(match[1], match[2], match[3])
      _alpha = ensureAlpha(match[4])
      _mode = RGB
    } else if ((match = matchers.hsl.exec(color))) {
      _color = ensureHSL(match[1], match[2], match[3])
      _mode = HSL
      ...
      _color = ensureRGB(rgb[1], rgb[2], rgb[3])
      _mode = RGB
  ...
  function fromObject(obj) {
    ...
    if (exists(obj.r) && exists(obj.g) && exists(obj.b)) {
      _color = ensureRGB(obj.r, obj.g, obj.b)
      _mode = RGB
    } else if (exists(obj.h) && exists(obj.s) && exists(obj.l)) {
      _color = ensureHSL(obj.h, obj.s, obj.l)
      _mode = HSL
    }
    if (exists(obj.a)) {
      _alpha = ensureAlpha(obj.a)
    }
  }

  function fromArray(arr, type=RGB) {
    if (typeof arr !== 'array') return
    if (exists(arr[0]) && exists(arr[1]) && exists(arr[2])) {
      if (type === HSL) {
        _color = ensureHSL(arr[0], arr[1], arr[2])
      } else {
        _color = ensureRGB(arr[0], arr[1], arr[2])
      }
      _mode = type
    }
    if (exists(arr[3])) {
      _alpha = ensureAlpha(arr[3])
    }
  }
